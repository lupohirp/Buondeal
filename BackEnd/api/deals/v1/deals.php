        ...
        try
        {
            ...
            elseif (isset($_GET['deal_id']))
            {
                ...
                foreach ($stmt['variants'] as $item)
                {
                    ...
                    $encoded_image_variants = array();
                    foreach($images_variants as $image){
                        $type = pathinfo($image->image_path, PATHINFO_EXTENSION);
                        $data = file_get_contents('file:///'.$image->image_path);
                        $base64 = 'data:image/' . $type . ';base64,' . base64_encode($data);

                        array_push($encoded_image_variants,$base64);
                    }

                    $item->{'images'} = $encoded_image_variants;
                    ...
                    foreach ($variants as $item)
                    {
                        ...
                        foreach ($images as $key => $item)
                        {
                            ...
                            if ($type == 'data:image/jpeg')
                            {
                                ...
                                $image->createThumbnail($temp_path . $key . ".jpg",400,400);

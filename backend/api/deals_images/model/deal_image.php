...
class DealImage
{
    ...
    private $conn;
    private $nome_tabella = "deals_images_variants_details";
    ...
    {
        try {
            $sql = "INSERT INTO " . $this->nome_tabella . " SET deal_id=:deal_id,variant_id=:variant_id,image_path=:image_path,zoomed_path=:zoomed_path";
            $stmt = $this->conn->prepare($sql);
            ...
            if ($stmt->execute()) {
               ...
               bd_log("Errore durante l'inserimento dell'immagine : " .implode(":",$stmt->errorInfo()), "e", basename(__FILE__));
               return false;
            }
        } catch (Exception $e) {
            echo json_response($e->getMessage(), 500);
